...
import { UserFormFields, fieldName, isRootAttribute, label } from "../../ui-shared/user-profile/utils";
...
export const UserProfileFields = ({
  ...
  const groupsWithAttributes = useMemo(() => {
    ...
    const attributes = hideReadOnly
      ? userProfileMetadata.attributes.filter(({ readOnly }) => !readOnly).map((attribute) => {return {
          ...attribute,
          required: false
        }})
      : userProfileMetadata.attributes.map((attribute) => {return {
        ...attribute,
          required: false
        }});
  ...
  console.log('groupsWithAttributes: ', groupsWithAttributes)
        ...
        .map(({ group, attributes }) => ({
          ...
          panel: (
            <div className="pf-v5-c-form">
              ...
              {attributes.filter((attribute) => (attribute.name === "email" || attribute.name == "firstName" || attribute.name == "lastName")).map((attribute) => (
                <FormField
                  key={attribute.name}
                  t={t}
                  form={form}
